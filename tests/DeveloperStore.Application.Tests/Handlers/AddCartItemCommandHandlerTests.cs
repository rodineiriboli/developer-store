...
namespace DeveloperStore.Application.Tests.Handlers
{
    ...
    public class AddCartItemCommandHandlerTests
    {
        ...
        public AddCartItemCommandHandlerTests()
        {
            ...
            _handler = new AddCartItemCommandHandler(_cartRepository, _unitOfWork, _mapper);
        }

        [Fact]
        public async Task Handle_ShouldAddItem_WhenCartExists()
        {
            // Arrange
        ...
        {
            // Arrange
            var command = new AddCartItemCommand { CartId = Guid.NewGuid() };
